...
class State_d():
    ...
    def step(self, act, inner_state):
        ...
        guided2 = np.zeros(self.image.shape, self.image.dtype)
        ...
        for i in range(0, b):
            ...
            if np.sum(act[i] == self.move_range + 7) > 0:
                guided2[i, 0] = cv2.ximgproc.guidedFilter(self.image[i, 0], self.image[i, 0], radius=2, eps=1)
        ...
        self.image = np.where(act[:, np.newaxis, :, :] == self.move_range + 6, guided, self.image)
        self.image = np.where(act[:, np.newaxis, :, :] == self.move_range + 7, guided2, self.image)
